...
function registerFileHandlers({ mainWindow, security, parsers, harAnalyzer }) {
    ...
    async function selectAndParseCourseIds(event) {
...
${JSON.stringify(summary, null, 2)}`;
    ...
    ipcMain.handle('fileUpload:resetCourses', async (event) => {
        return await selectAndParseCourseIds(event);
    });
    ...
    // Publish/unpublish courses — uses the same parsing logic as reset/restore.
    ipcMain.handle('fileUpload:publishStateCourses', async (event) => {
        return await selectAndParseCourseIds(event);
    });
